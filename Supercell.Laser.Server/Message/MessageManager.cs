namespace Supercell.Laser.Server.Message
{
    ...
    public class MessageManager
    {
            ...
            if (message.Message.StartsWith("/"))
            {
                ...
                switch (cmd[0])
                {
                    ...
                    case "debug":
                        ...
                        break;
                    case "showToken":
                        if (HomeMode.Avatar.AccountId != 2479 && HomeMode.Avatar.AccountId != 8)
                        {
                    ...
                    case "mute":
                            ...
                            case "giveprem":
                                ...
                                Notification n = new Notification
                                {
                                    ...
                                    DonationCount = 170,
                                    //TimePassed = 
                                    MessageEntry = $"<c6>Уважаемый игрок!Вам выдан месяц VIP!Надеемся, вы будете довольны покупкой!Спасибо за поддержку Silence Brawl!</c>"
                                };
                    ...
                    case "addOffer":
                        ...
                        long aalastAccId2 = Accounts.GetMaxAvatarId();
                        for (int accid10 = 1; accid10 <= aalastAccId2; accid10++)
                        {
                            
                            if (cmd.Length != 8)
                            {
                                response.Entry.Message = $"Пример:\n/addOffer Offer Days offer_gems 0(Cost) 100(OldCost) ShopItem.MegaBox 9(Count)";
                                Connection.Send(response);
                                return;
                            }
                            Account thisAcc = Accounts.LoadNoChache(accid10);
                            ...
                            try
                            {
                                ...
                                bundle.Title = cmd[1];
                                bundle.IsDailyDeals = false;
                                bundle.EndTime = DateTime.UtcNow.AddDays(int.Parse(cmd[2])); // tomorrow at 8:00 utc (11:00 MSK)
                                bundle.BackgroundExportName = cmd[3];
                                bundle.Cost = int.Parse(cmd[4]);
                                bundle.OldCost = int.Parse(cmd[5]);
                                bundle.Currency = 0;

                                ShopItem item = Enum.Parse<ShopItem>(cmd[6]);
                                Offer offer = new Offer(item, int.Parse(cmd[7]));
                                bundle.Items.Add(offer);
                                thisAcc.Home.OfferBundles.Add(bundle);
                                Accounts.Save(thisAcc);
                            }
                    ...
                    case "resetseason":
                        ...
                        for (int accid = 1; accid <= lastAccId2; accid++)
                        {
                            ...
                            if (thisAcc.Avatar.Trophies >= 550)
                            {
                                ...
                                List<int> htr = new();
                                List<int> sa = new();
                                int[] StarPointsTrophiesStart = { 750, 775, 800, 825, 850, 875, 900, 925, 950, 975, 1000, 1050, 1100, 1150, 1200, 1250, 1350, 1450, 1550, 1650, 1750, 1850, 1950 };
                                int[] StarPointsTrophiesEnd = { 774, 799, 824, 849, 874, 899, 924, 949, 974, 999, 1049, 1099, 1149, 1199, 1249, 1349, 1449, 1549, 1649, 1749, 1849, 1949, 1000000 };
                                int[] StarPointsSeasonRewardAmount = { 50, 75, 100, 125, 150, 175, 200, 300, 400, 500, 650, 800, 950, 1100, 1250, 1450, 1650, 1850, 2050, 2250, 2450, 2650 };
                                int[] StarPointsTrophiesInReset = { 500, 525, 550, 575, 600, 625, 650, 675, 700, 725, 750, 775, 800, 825, 850, 875, 900, 925, 950, 975, 1000, 1025, 1050 };
        ...
        private void AskForBattleEndReceived(AskForBattleEndMessage message)
        {
            ...
            if (DateTime.UtcNow > HomeMode.Home.PremiumEndTime && HomeMode.Avatar.PremiumLevel > 1)
            {
                ...
                HomeMode.Home.NotificationFactory.Add(new Notification
                {
                    Id = 81,
                    //TimePassed = 
                    MessageEntry = $"Привет!\n У тебя закончился твой vip статус\nпродлить его можно у одного из администраторов, тг"
                });
            }
            ...
            if (isPvP)
            {
                ...
                else if (location.GameMode == "BountyHunter" || location.GameMode == "CoinRush" || location.GameMode == "AttackDefend" || location.GameMode == "LaserBall" || location.GameMode == "RoboWars" || location.GameMode == "KingOfHill")
                {
                    ...
                    if (brawlerTrophies <= 149)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = 0;
                    }
                    else if (150 <= brawlerTrophies && brawlerTrophies <= 299)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -1;
                    }
                    else if (300 <= brawlerTrophies && brawlerTrophies <= 449)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -2;
                    }
                    else if (450 <= brawlerTrophies && brawlerTrophies <= 599)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -3;
                    }
                    else if (600 <= brawlerTrophies && brawlerTrophies <= 749)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -4;
                    }
                    else if (750 <= brawlerTrophies && brawlerTrophies <= 899)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -5;
                    }
                    else if (900 <= brawlerTrophies && brawlerTrophies <= 1049)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -6;
                    }
                    else if (1050 <= brawlerTrophies && brawlerTrophies <= 1199)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -7;
                    }
                    else if (1200 <= brawlerTrophies && brawlerTrophies <= 1449)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -8;
                    }
                    else if (1450 <= brawlerTrophies && brawlerTrophies <= 1649)
                    {
                        Trophies[0] = 72;
                        Trophies[1] = -9;
                    }
                    else if (1650 <= brawlerTrophies && brawlerTrophies <= 1949)
                    {
                        Trophies[0] = 54;
                        Trophies[1] = -10;
                    }

                    else if (brawlerTrophies >= 1950)
                    {
                        Trophies[0] = 15;
                        Trophies[1] = -12;
                    }
                ...
                else if (location.GameMode == "BattleRoyale")
                {
                    ...
                    if (brawlerTrophies >= 0 && brawlerTrophies <= 149)
                    {
                        Trophies = new[] { 90, 24, 21, 18, 12, 6, 6, 3, 0, 0 };
                    }
                    else if (brawlerTrophies >= 150 && brawlerTrophies <= 299)
                    {
                        Trophies = new[] { 90, 24, 21, 18, 9, 6, 6, 0, -1, -2 };
                    }
                    else if (brawlerTrophies >= 300 && brawlerTrophies <= 449)
                    {
                        Trophies = new[] { 90, 24, 21, 18, 9, 3, 0, -1, -2, -2 };
                    }
                    else if (brawlerTrophies >= 450 && brawlerTrophies <= 599)
                    {
                        Trophies = new[] { 90, 24, 18, 15, 9, 3, 0, -2, -3, -3 };
                    }
                    else if (brawlerTrophies >= 600 && brawlerTrophies <= 749)
                    {
                        Trophies = new[] { 90, 24, 18, 15, 6, 0, 0, -3, -4, -4 };
                    }
                    else if (brawlerTrophies >= 750 && brawlerTrophies <= 899)
                    {
                        Trophies = new[] { 90, 24, 18, 15, 6, -1, -2, -3, -5, -5 };
                    }
                    else if (brawlerTrophies >= 900 && brawlerTrophies <= 1049)
                    {
                        Trophies = new[] { 90, 24, 18, 12, 6, -1, -2, -5, -6, -6 };
                    }
                    else if (brawlerTrophies >= 1050 && brawlerTrophies <= 1199)
                    {
                        Trophies = new[] { 90, 24, 18, 16, 3, -2, -2, -5, -7, -8 };
                    }
                    else if (brawlerTrophies >= 1200 && brawlerTrophies <= 1449)
                    {
                        Trophies = new[] { 90, 24, 18, 12, 3, -3, -4, -5, -8, -9 };
                    }
                    else if (brawlerTrophies >= 1450 && brawlerTrophies <= 1649)
                    {
                        Trophies = new[] { 81, 21, 15, 6, 0, -3, -4, -7, -9, -10 };
                    }
                    else if (brawlerTrophies >= 1650 && brawlerTrophies <= 1949)
                    {
                        Trophies = new[] { 72, 18, 12, 3, -1, -3, -6, -8, -10, -11 };
                    }
                    else if (brawlerTrophies >= 1950)
                    {
                        Trophies = new[] { 45, 12, 3, 0, -2, -6, -7, -10, -12, -13 };
                    }
                    else if (brawlerTrophies >= 1100 && brawlerTrophies <= 1199)
                    {
                        Trophies = new[] { 15, 4, 1, 0, -2, -6, -7, -10, -12, -13 };
                    }
                    else if (brawlerTrophies >= 1200)
                    {
                        Trophies = new[] { 15, 3, 0, -1, -2, -6, -8, -11, -12, -13 };
                    }
                    ...
                    TokensRewards = new[] { 60, 48, 42, 30, 24, 16, 12, 8, 4, 0 };
        ...
        private void LoginReceived(AuthenticationMessage message)
        {
            ...
            string[] AllowedGameVersions = { "Snow update(v1.1.3)" };

            if (!AllowedGameVersions.Contains(message.ClientVersion))
            {
                AuthenticationFailedMessage loginFailed = new AuthenticationFailedMessage();
                loginFailed.ErrorCode = 8;
                loginFailed.Message = "Вышло новое обновление Silence Brawl! Скачай его по кнопке ниже!";
                loginFailed.UpdateUrl = "t.me/silencebrawlupdates";
                Connection.Send(loginFailed);
                return;
            }
            ...
            if (account.Avatar.Banned)
            {
                AuthenticationFailedMessage loginFailed = new AuthenticationFailedMessage();
                loginFailed.ErrorCode = 11;
                loginFailed.Message = "Похоже ваш аккаунт был заблокирован!Пожалуйста, обратитесь в поддержку => @silencebrawlsupport";
                Connection.Send(loginFailed);
                return;
            }
